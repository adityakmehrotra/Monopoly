...
public class CommunityChest {
	private Stack<Integer> communityChestCardStack;

	public CommunityChest() throws FileNotFoundException, IOException, ParseException, JSONException {
		this.communityChestCardStack = generateStack();
	}
	 ...
	 * stack is the index of all Community Chest cards.
	 * 
	 * @return Stack of Integers that randomizes 0-14 (inclusive)
	 */
	public Stack<Integer> generateStack() {
		Random random = new Random();
		Stack<Integer> communityChestOrder = new Stack<Integer>();
		...
		while (cards.size() != 0) {
			...
			communityChestOrder.push(cards.get(choice));
			cards.remove(choice);
		}

		return communityChestOrder;
	}

	public Stack<Integer> getCommunityChestCardStack() {
		return communityChestCardStack;
	}

	public int getCommunityChestIndex() {
		int communityChestIndex = communityChestCardStack.pop();

		if (communityChestCardStack.size() == 0) {
			communityChestCardStack = generateStack();
		}
	...
	public String getTitle(Integer index) throws FileNotFoundException, JSONException, IOException, ParseException {
		...
		JSONObject propertyDetails = (JSONObject) ((JSONArray) jsonObject.get("Community Chest Cards")).get(index);
